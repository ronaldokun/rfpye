...
__all__ = ['path_type', 'bin_val', 'bytes_encoded', 'datetime_object', 'create_base_block', 'public_attrs', 'getattrs',
           'BASEBLOCK', 'TIMEDBLOCK', 'EXCLUDE_ATTRS', 'DICT_PROCESSING', 'DICT_UNIT', 'BaseBlock', 'TimedBlock',
...
def timedblock(base_block: NamedTuple)->NamedTuple:
    date = bin2date(base_block.data[:4])
    time = bin2time(base_block.data[4:8])
    nanosecs = bin2dec(base_block.data[8:12])
    start_mega = bin2dec(base_block.data[12:14])
    stop_mega = bin2dec(base_block.data[18:20])
    start_mili = bin2dec(base_block.data[14:18])
    stop_mili = bin2dec(base_block.data[20:24])
    channel_start = bin2dec(base_block.data[24:26])
    channel_end = bin2dec(base_block)
    datetime_stamp = datetime(2000+date[2], date[1], date[0],
                              time[0], time[1], time[2], int(nanosecs/1000))
    return namedtuple('TimeBlock', )
...
#         return np.asarray([self[i] for i in range(len(self))], dtype=np.float32)
    ...
    def block_data(self) -> np.array:
        """Retorna o array com as medidas de espectro do bloco em 'dB'.
