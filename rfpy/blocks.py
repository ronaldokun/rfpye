...
class DType65(GetAttr):
    ...
    *F8 = (2u bytes) STOPCHAN = Stop Channel number
    ...
    def block_data(self) -> np.array:
        ...
        return (
            np.frombuffer(self.data[start:end], dtype=np.float16, count=stop - start)
            / 2
        )
...
class DType67(GetAttr):
    ...
    def block_data(self) -> np.array:
        return (
            np.frombuffer(
                self.data[self.start : self.stop], dtype=np.float16, count=stop - start
            )
            / 2
            + self.offset
            - 127.5
...
class DType69(GetAttr):
    ...
    def block_data(self) -> np.array:
        ...
        return np.frombuffer(
            self.data[start:stop], dtype=np.float16, count=stop - start
        )
