...
__all__ = ['path_type', 'bin_val', 'bytes_encoded', 'datetime_object', 'create_base_block', 'BaseBlock',
           'TimedNonSpectral', 'TimedSpectral', 'TimedVersion5', 'DType6', 'DType20', 'DType21', 'DType24', 'DType40',
           'DType41', 'DType42', 'DType51', 'DType63', 'DType64', 'DType65', 'DType66', 'DType67', 'DType68', 'DType69',
...
class TimedSpectral(GetAttr):
    ...
    @property
    def stop_mega(self): return bin2int(self.data[BYTES_TIMED[5]], False) #F5
    @property
    def start_mili(self): return bin2int(self.data[BYTES_TIMED[4]])
    @property
    def stop_mili(self): return bin2int(self.data[BYTES_TIMED[6]])
    @property
    def start_channel(self): return bin2int(self.data[BYTES_TIMED[7]], False)
    @property
...
class TimedVersion5(GetAttr):
    ...
    @property
    def group_id(self)->int: return bin2int(self.data[BYTES_V5[3]])
    @property
    def dynamic_id(self)->int: return bin2int(self.data[BYTES_V5[4]])
    @property
    def desclen(self)->int: return bin2int(self.data[BYTES_V5[5]])
...
class DType20(GetAttr):
    ...
    def __init__(self, block: NamedTuple):
        self.default = block
    @property
    def bottom_latitude(self)->float: return bin2int(self.data[BYTES_20[0]]) / 1e6
    @property
    def left_longitude(self)->float: return bin2int(self.data[BYTES_20[1]]) / 1e6
    @property
    def top_latitude(self)->float: return bin2int(self.data[BYTES_20[2]]) / 1e6
    @property
    def right_longitude(self)->float: return bin2int(self.data[BYTES_20[3]]) / 1e6
    @property
    ...
    def spectral_blocks_info(self)->L:
        ...
        step = 8
        return L.range(start, stop, step).map(lambda i: (bin2int(self.data[i:i+4]), bin2int(self.data[i+4:i+8])))
        #return L((self.data[i:i+4], self.data[i+4:i+8]) for i in range(start, stop, step))
...
class DType51(GetAttr):
    ...
    @property
    def desclen(self): return bin2int(self.data[BYTES_51[5]])
    @property
...
class DType63(GetAttr):
    ...
    def tunning_info(self) -> str:
        ...
        return tuple(f'{bin2int(self.data[i:i+2])}MHz-{TUNING_BLOCK.get(self.data[i+3], 0)}' for i in range(start, stop, 4))
    @property
