...
def create_base_block(bin_block: bytes)->Tuple:
    ...
    return BaseBlock(*L(bin_block[:4], bin_block[4:8], bin_block[8:12]).map(bin2int), bin_block[12:-4])
...
class DType20(GetAttr):
    ...
    def spectral_blocks_info(self)->L:
        return L((self.data[i:i+4], self.data[i+4:i+8]) for i in range(BYTES_20[9].stop, BYTES_20[9].stop + self.nddt, 8))
...
class DType21(GetAttr):
    ...
    def _get_text2_len(self) -> int:
        ...
        final = 4 + inicio
        return bin2int(self.data[inicio:final])
...
class DType63(GetAttr):
    ...
    def tunning_info(self) -> str:
        ...
        return {f'{bin2int(self.data[i:i+2])}MHz':TUNING_BLOCK.get(self.data[i+3], 0) for i in range(start, stop, 4)}
